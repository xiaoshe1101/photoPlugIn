...
//        var imgs = $(page.parent).find('img').filter(function(){
//            return ($(this).attr("src") != "");
//        }),nowimg = imgs.eq(page.start);
        var imgs = $(options.page.imgList),nowimg = imgs.eq(page.start);
    ...
    var conf = {
        ...
        bgcolor: '',
        page: {
            html: '<div class="xubox_bigimg"><img src="'+ log.thissrc +'" alt="'+ (log.name || '') +'" layer-pid="'+ (log.pid || '') +'"><p class="xz-btn-box"><button class="turn-left">向左旋转</button><button class="turn-right">向右旋转</button></p><div class="xubox_imgsee">'+ function(){
                if(log.imgLen > 1){
            ...
            $(".turn-right").click(function(){
                ...
                img.css({"transform":"rotate("+current+"deg)",
                        "-ms-transform":"rotate("+current+"deg)",     /* IE 9 */
                        "-moz-transform":"rotate("+current+"deg)",   /* Firefox */
                        "-webkit-transform":"rotate("+current+"deg)", /* Safari 和 Chrome */
...
layer.photosPage = function(options){
    ...
    options = options || {};
    log.run(options.index);
};

/**
 * 调用相册的方法,传入需要显示的所有的图片,把src为空得图片删除
 * @param imgs
 */
